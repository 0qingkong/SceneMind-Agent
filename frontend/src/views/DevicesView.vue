...
import { PhCamera, PhDeviceMobileCamera, PhEyeglasses, PhUploadSimple } from '@phosphor-icons/vue'
  ...
  <section class="devices-view">
    ...
    <div v-else class="device-grid">
      <article class="device-card">
        <span class="device-icon"><PhUploadSimple :size="24" weight="duotone" /></span><div><h2>图片上传</h2><p>文件选择来源</p></div>
      ...
      <article class="device-card">
        <span class="device-icon"><PhCamera :size="24" weight="duotone" /></span><div><h2>浏览器摄像头</h2><p>{{ cameras.length ? `发现 ${cameras.length} 个视频输入` : '需要权限后显示设备名称' }}</p></div>
      ...
      <article v-for="item in cameras" :key="item.id" class="device-card camera-device-card">
        <span class="device-icon"><PhDeviceMobileCamera :size="24" weight="duotone" /></span><div><h2>{{ item.label }}</h2><p>视频输入 {{ cameras.indexOf(item) + 1 }}</p></div>
      ...
      <article class="device-card simulator-device-card">
        <span class="device-icon"><PhEyeglasses :size="24" weight="duotone" /></span><div><h2>AI Glasses Simulator</h2><p>未来设备交互预览</p></div>
