  ...
  <section class="sessions-view">
    ...
    <section class="session-create-card">
      ...
      <div class="session-form-grid">
        <label>会话标题<input v-model="title" maxlength="200" placeholder="例如：实验室巡查（可选）" /></label>
        <label>地点<input v-model="location" maxlength="200" placeholder="例如：实验室（可选）" /></label>
        <label>关注物体<input v-model="targetQuery" maxlength="200" placeholder="例如：杯子（可选）" /></label>
    ...
    <div v-else-if="sessions.length" class="session-list">
      <RouterLink v-for="item in sessions" :key="item.id" class="session-card" :to="`/sessions/${item.id}`">
        <div><span class="session-status" :class="item.status">{{ item.status === 'active' ? '运行中' : item.status === 'failed' ? '失败' : '已停止' }}</span><h3>{{ item.title || '未命名观察会话' }}</h3></div>
